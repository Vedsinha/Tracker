...
const emit = defineEmits<{
  ...
  (e: "toggle", payload: { id: Id<"tasks">; done: boolean }): void;
}>();
...
<template>
  <div
    class="mb-0 flex h-full w-full flex-col gap-3 rounded-3xl border-2 border-black bg-white px-6 py-6 shadow-[0_18px_60px_-32px_rgba(0,0,0,0.65)]"
  >
    ...
    <div
      v-if="editable"
      class="rounded-2xl border border-black/10 bg-black/5 px-3 py-2"
    >
      <form class="flex items-center gap-2" @submit.prevent="handleAdd">
        <input
          ...
          class="flex-1 bg-transparent text-sm text-black outline-none placeholder:text-neutral-500"
        />
        <button
          type="submit"
          class="rounded-full bg-black px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-white shadow-[0_10px_25px_-18px_rgba(0,0,0,0.8)] transition hover:-translate-y-[1px]"
        >
          Add
        </button>
      </form>
